...
import { isFrameElement, isFrameLikeElement, isArrowElement } from "./element/typeChecks";
  ...
  try {
    ...
    for (const element of oldElements) {
      try {
        if (element.frameId) {
          ...
          if (nextElementId) {
            ...
            if (nextElement) {
              ...
              const frameElement = nextFrameId ? nextElementMap.get(nextFrameId) : null;
              // Create or update customData property with "Frame"
              const newCustomData = nextElement.customData ? { ...nextElement.customData } : {};
              
              // Remove old "Frame id" property if it exists (for backwards compatibility)
              if ("Frame id" in newCustomData) {
                delete newCustomData["Frame id"];
              }
              
              // Set the Frame custom property to the frame's name, or a default name if not available
              if (frameElement && isFrameLikeElement(frameElement)) {
                newCustomData["Frame"] = frameElement.name || getFrameLikeTitle(frameElement);
              } else {
                newCustomData["Frame"] = "Frame";
              }
              
              // Ensure Frame property is never empty
              if (!newCustomData["Frame"] || newCustomData["Frame"].trim() === "") {
                newCustomData["Frame"] = "Frame";
              }
        ...
        console.error("Error binding element to frame:", element.id, err);
      }
    }
  } catch (err) {
    console.error("Error in bindElementsToFramesAfterDuplication:", err);
  }
};
...
export const addElementsToFrame = <T extends ElementsMapOrArray>(
  ...
  for (const element of finalElementsToAdd) {
    try {
      // Create or update customData property with "Frame"
      const newCustomData = element.customData ? { ...element.customData } : {};
      ...
      // Set the Frame custom property to the frame's name, or a default name if not available
      newCustomData["Frame"] = frame.name || getFrameLikeTitle(frame);
      ...
      console.error("Error adding element to frame:", element.id, err);
    }
  }
...
export const removeElementsFromFrame = (
  ...
  for (const [, element] of _elementsToRemove) {
    // Remove both "Frame" and "Frame id" from customData for backwards compatibility
    let newCustomData = element.customData ? { ...element.customData } : undefined;
    if (newCustomData) {
      // Remove "Frame" property with any case
      const frameKey = Object.keys(newCustomData).find(
        (key) => key.toLowerCase() === "frame"
      );
      
      if (frameKey) {
        delete newCustomData[frameKey];
      }
      
      // Also delete old "Frame id" property if it exists
      if ("Frame id" in newCustomData) {
        delete newCustomData["Frame id"];
      }
      
      // If customData is now empty, set it to undefined
...
export const shouldApplyFrameClip = (
  ...
  // Always apply clipping for arrows, regardless of frame clipping setting
  if (!appState.frameRendering || (!appState.frameRendering.clip && !isArrowElement(element))) {
    return false;
  }

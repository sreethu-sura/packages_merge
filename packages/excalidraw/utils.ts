...
// Coordinate system scale - change this value to adjust the scale
...
export const viewportCoordsToSceneCoords = (
  ...
  // Apply COORDINATE_SCALE but keep default canvas coordinates (Y down)
  const x = ((clientX - offsetLeft) / zoom.value - scrollX) * COORDINATE_SCALE;
  const y = ((clientY - offsetTop) / zoom.value - scrollY) * COORDINATE_SCALE;
...
export const sceneCoordsToViewportCoords = (
  ...
  // Apply COORDINATE_SCALE but keep default canvas coordinates (Y down)
  const x = (sceneX / COORDINATE_SCALE + scrollX) * zoom.value + offsetLeft;
  const y = (sceneY / COORDINATE_SCALE + scrollY) * zoom.value + offsetTop;
...
export const getZoomMultiplier = ({
  ...
  defaultValue: number;
  currentZoom: Zoom;
}) => (defaultValue * (currentZoom.value - 0.1)) / currentZoom.value;

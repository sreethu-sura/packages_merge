...
class Scene {
  ...
  getNonDeletedElementsMap() {
    const nonHiddenElementsMap = new Map(this.nonDeletedElementsMap);
    for (const [id, element] of this.nonDeletedElementsMap) {
      if (element.hidden === true) {
        nonHiddenElementsMap.delete(id);
      }
    }
    return toBrandedType<NonDeletedSceneElementsMap>(nonHiddenElementsMap);
  }
